...
def Fix_Special(url):
    dialogprocess.create("TeQ IQ IPTV","Renaming paths...",'', 'Please Wait')
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dialogprocess.create("TeQ IQ IPTV","Archiving...",'', 'Please Wait')
...
def Destroy_Path(path):
    dialogprocess.create("TeQ IQ IPTV ","Cleaning...",'', 'Please Wait')
...
def Check_Path():
    if zip=='':
     if dialog.yesno("TeQ IQ IPTV Wizard", "You Have Not Set Your Storage Path", 'Set The Storage Path Now ?',''):
        ADDON.openSettings()
        print '######### ZIP DIRECTORY #########'
...
def RestoreIt():
    ...
    dialog = xbmcgui.Dialog()
    if zip == '':
        dialog.ok('TeQ IQ IPTV Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    dialogprocess.create("TeQ IQ IPTV Wizard","Restoring",'', 'Please Wait')   
    lib=xbmc.translatePath(os.path.join(zip,'backup.zip'))
    Read_Zip(lib)
    dialogprocess.create("TeQ IQ IPTV Wizard","Checking ",'', 'Please Wait')
    ...
    dialogprocess.close()
    dialog.ok("TeQ IQ IPTV Wizard", "All Done, DONT PRESS OK", "Wait a 5 minutes and pull the Power","")	
def Backupzip():  
    if zip == '':
        dialog.ok('TeQ IQ IPTV Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    ...
    dialogprocess.create("TeQ IQ IPTV Wizard","Backing Up",'', 'Please Wait')
    ...
    zipobj.close()
    dialogprocess.close()
    dialog.ok("TeQ IQ IPTV Wizard", "You Are Now Backed Up", '','')
